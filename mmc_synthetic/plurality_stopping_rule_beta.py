...
class BernoulliMixtureEProcess:
    ...
    Uses a discrete Beta(a,b) mixture truncated to (1/2,1].
    """
    qs: List[float]
    ws: List[float]
    s: int = 0
    ...
    def update(self, y: int) -> float:
        ...
        self.f += (1 - y)
        m = self.s + self.f
        # e_n = 2^m * sum_k w_k * q_k^s * (1-q_k)^f
        log_terms = [math.log(w) + self.s * math.log(q) + self.f * math.log1p(-q)
                     for (q, w) in zip(self.qs, self.ws)]
        self.log_e = m * math.log(2.0) + logsumexp(log_terms)
        return self.log_e
...
class PluralityEAndDecider:
    ...
    def __init__(
        ...
        qs, ws = make_trunc_beta_grid(a=a, b=b, K=grid_K, lo=0.5001, hi=0.9999)
        self.pair_E = BernoulliMixtureEProcess(qs, ws)
        self.other_E = BernoulliMixtureEProcess(qs, ws)
    ...
    def on_vote(self, label: Any) -> Dict[str, Any]:
        ...
        # 1) Predictable selection from t-1
