# plurality_stopping_rule_prior.py
"""
...
class BernoulliMixtureEProcess:
    ...
    def update(self, y: int) -> float:
        ...
        self.f += (1 - y)
        m = self.s + self.f
        # e_n = e_{n-1} * 2 * qdirac^y * (1-qdirac)^(1-y)
        self.log_e += math.log(2.0) + y * math.log(self.qdirac) + (1 - y) * math.log1p(-self.qdirac)
        return self.log_e
...
class PluralityEAndDeciderPriorRatioUpdate:
    """
